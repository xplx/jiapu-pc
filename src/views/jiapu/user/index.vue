...
<script lang="ts" name="jiapu-user" setup>
  import { BasicTable, TableAction } from '/@/components/Table';
  import { useModal } from '/@/components/Modal';
  import UserModal from './UserModal.vue';
  import { useListPage } from '/@/hooks/system/useListPage';
  import { columns, searchFormSchema } from './user.data';
  import { getList, deleteUser, batchDeleteUser } from './user.api';

  const [registerModal, { openModal }] = useModal();

  const { tableContext, doRequest } = useListPage({
    designScope: 'jiapu-user',
    tableProps: {
      title: '用户管理',
      api: getList,
      columns: columns,
  ...
  async function batchHandleDelete() {
    doRequest(() => batchDeleteUser({ ids: selectedRowKeys.value }));
  }
